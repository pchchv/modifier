	...
	for i := 0; i < len(tags); i++ {
		...
		if tagsVal, found := t.aliases[tg]; found {
			if i == 0 {
				...
				next, curr, e := t.parseFieldTagsRecursive(tagsVal, fieldName, tg, true)
				if e != nil {
					err = e
					return
				}
		...
		case endKeysTag:
			...
			// if there are more in tags then there was no keysTag defined and an error should be thrown
			if i != len(tags)-1 {
